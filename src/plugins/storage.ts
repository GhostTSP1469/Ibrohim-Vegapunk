...
async function storagePlugin(app: FastifyInstance): Promise<void> {
  ...
  await app.register(async (scope) => {
    ...
    // Internal, unversioned routes (not under `/api/v1`) — hidden from the
    // OpenAPI doc so they don't get a wrong base path in the published servers.
    scope.put('/internal/storage/*', { schema: { hide: true } }, async (request, reply) => {
    ...
    scope.get('/internal/storage/*', { schema: { hide: true } }, async (request, reply) => {
      const key = (request.params as Record<string, string>)['*'] ?? '';
      let stream;
      try {
