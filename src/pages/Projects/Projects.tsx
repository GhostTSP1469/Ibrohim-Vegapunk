...
export default function Projects() {
  ...
  // Project-members modal state (add member by email → workspace + project).
  ...
  const onAddFound = async () => {
    ...
    // workspace first (ignored if they already are), then the project.
  ...
  return (
    <div className="space-y-5">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between border-t border-gray-100 px-4 py-2.5">
                  ...
                  <button
                    onClick={() => openMembers(p)}
                    className="inline-flex items-center gap-1 text-xs font-medium text-gray-400 transition-colors hover:text-emerald-600"
                  >
      ...
      {modalProject && (
        ...
        >
          <div
            className="pj-modal w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-800">
                <Users size={18} className="text-emerald-600" /> {modalProject.name}
              </h2>
              <button
                onClick={() => setModalProject(null)}
            ...
            <form onSubmit={onLookup} className="flex items-center gap-2">
              <input
                className="flex-1 rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-gray-800 outline-none transition-all duration-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20"
                type="email"
              ...
              <button
                type="submit"
                disabled={busy}
                className="rounded-xl bg-[#131a2e] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#1c274a] disabled:opacity-60"
              >
                {busy ? "…" : "Find"}
              </button>
            </form>
            ...
            {lookupMsg && <p className="mt-2 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-600">{lookupMsg}</p>}
              ...
              <ul className="max-h-56 divide-y divide-gray-100 overflow-y-auto rounded-xl border border-gray-100">
                {members.map((m) => (
                  <li key={m.user_id} className="flex items-center justify-between px-3 py-2">
                    ...
                    <button
                      onClick={() => removeMember(workspaceSlug, modalProject.id, m.user_id)}
            ...
            <div className="mt-5 flex justify-end">
              <button
                onClick={() => setModalProject(null)}
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-slate-50"
              >
                <Check size={15} /> Done
              </button>
            </div>
