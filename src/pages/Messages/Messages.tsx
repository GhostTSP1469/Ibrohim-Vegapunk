...
export default function Messages() {
  ...
  // Poll the open conversation so incoming messages appear without a refresh.
  useEffect(() => {
    if (!activeId) return;
    const t = setInterval(() => {
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-gray-50/40">
          {active ? (
            <>
              ...
              <div className="flex flex-1 flex-col gap-1.5 overflow-y-auto overflow-x-hidden p-4">
                {messages.map((m) => {
                  const mine = m.sender_id === me;
                  return (
                    <div
                      key={m.id}
                      className={`max-w-[75%] wrap-break-word rounded-2xl px-3.5 py-2 text-sm shadow-sm ${mine ? "self-end bg-brand-600 text-white" : "self-start bg-white text-gray-800"}`}
                    >
                      {m.body}
                    </div>
                  );
              ...
              <form onSubmit={onSend} className="flex items-center gap-2 border-t border-gray-100 bg-white p-3">
                <input className="min-w-0 flex-1 rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm outline-none focus:border-brand-500 focus:bg-white" placeholder="Type a message…" value={body} onChange={(e) => setBody(e.target.value)} />
                <button type="submit" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-600 text-white transition hover:bg-brand-700 disabled:opacity-50" disabled={!body.trim()}>
                  <Send size={16} />
                </button>
              </form>
            </>
